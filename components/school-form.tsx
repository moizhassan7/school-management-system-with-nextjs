...
const studentFormSchema = z.object({
    ...
    // FIXED: Made email required to match database constraint
    parentEmail: z.string().email('Parent email is required for login'),
...
export default function StudentForm() {
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            if (!studentRecordId) {
                throw new Error("Student created but record ID missing - check API");
            }
                            ...
                            <div className="border-t border-slate-200 pt-6 mt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {form.watch('parentMode') === 'CREATE' && !selectedParent && (
                                    <>
                                        ...
                                        {/* FIXED: Removed (Optional) text and made field required in schema */}
                                        <FormField control={form.control} name="parentEmail" render={({ field }) => (
                                            <FormItem><FormLabel className="text-slate-700">Email Address *</FormLabel><FormControl><Input {...field} className="bg-white" /></FormControl><FormMessage /></FormItem>
                                        )} />
                                    </>
                                )}
                            </div>
